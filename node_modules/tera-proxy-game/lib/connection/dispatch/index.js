...
class Dispatch {
	constructor(connection, protocolVersion = 0) {
        ...
        let messages = this.is_console() ? this.protocol.console_messages : this.protocol.messages;
        if(messages) {
	...
	write(outgoing, name, version, data) {
		...
		if(Buffer.isBuffer(name)) {
			...
			try {
				data = this.protocol.write(this.protocolVersion, normalizedName, version, data, null, null, null, this.is_console())
			} catch (e) {
				log.error([
	...
	handle(data, incoming, fake = false) {
		...
		if(code === 19900 && !this.protocolVersion) { // C_CHECK_VERSION
			...
			try {
				const parsed = this.protocol.parse(ver, code, 1, data, null, this.is_console()),
					[item] = parsed.version
		...
		for(const hook of iterateHooks(globalHooks, codeHooks)) {
			...
			if(hook.definitionVersion === 'raw')
				...
				try {
					...
					let event = eventCache[defVersion] || (eventCache[defVersion] = this.protocol.parse(protocolVersion, code, defVersion, data, null, this.is_console()))
					...
					try {
						...
						if(result === true) {
							...
							try {
								data = this.protocol.write(protocolVersion, code, defVersion, event, null, null, null, this.is_console())
								bufferAttachFlags(data)
