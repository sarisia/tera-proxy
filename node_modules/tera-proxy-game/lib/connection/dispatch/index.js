...
class Dispatch extends EventEmitter {
	constructor(connection, protocolVersion = 0) {
		...
		let messages = this.isConsole() ? this.protocol.console_messages : this.protocol.messages;
		if(messages) {
			for(const [name, defs] of messages) {
	...
	isConsole() {
		return this.connection.info.console ? true : false;
	}
	...
	parseSystemMessage(message) {
		...
		const tokens = message.split('\v'),
			id = parseInt(tokens[0].substring(1)),
			name = this.sysmsgMap.code.get(id)
	...
	buildSystemMessage(message, data) {
		...
		const id = this.sysmsgMap.name.get(message.id)
	...
	createHook(base, name, version, opts, cb) {
		...
		if(name === '*') {
			...
			if(version !== '*') {
				const latest = this.latestDefVersion.get(normalizedName)
	...
	write(outgoing, name, version, data) {
		...
		if(Buffer.isBuffer(name)) {
			...
			try {
				data = this.protocol.write(this.protocolVersion, normalizedName, version, data, null, null, null, this.isConsole())
			} catch (e) {
				log.error([
	...
	setProtocolVersion(version) {
		...
		this.majorPatchVersion = this.connection.info.majorPatchVersions[this.protocolVersion]
	...
	handle(data, incoming, fake = false) {
		...
		if(code === 19900 && !this.protocolVersion) { // C_CHECK_VERSION
			...
			try {
				const parsed = this.protocol.parse(ver, code, 1, data, null, this.isConsole()),
					[item] = parsed.version
		...
		for(const hook of iterateHooks(globalHooks, codeHooks)) {
			...
			if(hook.definitionVersion === 'raw')
				...
				try {
					...
					let event = eventCache[defVersion] || (eventCache[defVersion] = this.protocol.parse(protocolVersion, code, defVersion, data, null, this.isConsole()))
					...
					try {
						...
						if(result === true) {
							...
							try {
								data = this.protocol.write(protocolVersion, code, defVersion, event, null, null, null, this.isConsole())
								bufferAttachFlags(data)
...
function deepClone(obj) {
	...
	if(obj instanceof Vec3) return obj.clone()
