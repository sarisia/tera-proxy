...
class DispatchWrapper {
  ...
  hookOnce(...args) {
    ...
    const self = this;
    const hook = this.base.hook(...args, function() {
      self.base.unhook(hook);
  ...
  send(name, version, data) {
    switch(name[0]) {
      case 'S':
      case 'I':
        this.toClient(name, version, data);
        break;
      case 'C':
        this.toServer(name, version, data);
        break;
      default:
        throw new Error(`Unrecognized packet opcode ${name}`);
    }
  }
