...
class TeraProtocol {
   ...
   */
  resolveIdentifier(protocolVersion, identifier, definitionVersion = '*', defaultName = '<Object>', definitionConsole = false) {
    const { maps, messages, console_messages, loaded } = this;
    let name;
    let code;
    let version;
    let definition;
    ...
    if (Array.isArray(identifier)) {
      ...
      const versions = (definitionConsole ? console_messages : messages).get(name);
      if (versions) {
  ...
  load(basePath = require.resolve('tera-data')) {
    const { maps, messages, console_messages } = this;
    ...
    for (const file of defFiles) {
      ...
      const parsedName = path.basename(file).match(/^(\w+)\.(\d+)(\.(pc|con))?\.def$/);
      if (!parsedName) {
      ...
      if(!platform || platform == '.con') {
        if (!console_messages.has(name)) console_messages.set(name, new Map());
        console_messages.get(name).set(version, definition);
      }
   ...
   * @returns {Object}
   */
  parse(protocolVersion, identifier, definitionVersion, reader, customName, definitionConsole) {
    // parse params
    ...
    const { name, version, definition } =
      this.resolveIdentifier(protocolVersion, identifier, definitionVersion, customName, definitionConsole);
   ...
   * @returns {Buffer}
   */
  write(protocolVersion, identifier, definitionVersion, data, writer, customName, customCode, definitionConsole) {
    // parse args
